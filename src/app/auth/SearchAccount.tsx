            ...
            <div className="h-full">
                ...
                <div>
                    <form className="flex flex-col items-center ">
                        <input onChange={(e)=>Setuser((user)=>({...user,email:e.target.value}))} value={user.email} type="email" placeholder={t("Enter")+t("Email")} className="border border-[#8790a2] h-10 w-4/5 mt-2 mb-2 px-2 showDiv"/>

                        <button type={"button"} onClick={ checkEmailExist} className="bg-[#f34d01e6] border border-[#8790a2] h-10 w-4/5"><span className="text-white">{t("Search")}</span></button>

                    </form>
                </div>
                <div className="text-center mt-4 mb-5 cursor-pointer">
                    <span className="mr-1 hover:text-[#f34d01e6]"><a onClick={Forgot}>{t("RemAccount")}</a></span>
                </div>
