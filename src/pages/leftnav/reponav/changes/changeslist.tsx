...
const EntryRow = ({repo, reference, entry}) => {
    ...
    const ChildEntryRow = ()=>{
      if(show && !children && entry.is_dir){
        const {response,loading,error} = useAPI(()=>{return repos.getEntriesInRef(user, repo.name, { path: (entry.fathername?entry.fathername+ '/':'' ) + entry.name,type: 'wip', ref: reference.name });})
